    ...
    handleCreateFlashcard = (note) => {
        ...
        .then((response) => {
            ...
            response.data.flashcards.forEach(flashcard => {
                ...
                axios.post('http://localhost:3002/notes/', flashcard)
                .then(saveResponse => {
                ...
                <Table striped bordered hover className={tableStyle}>
                    ...
                    <tbody>
                        {this.state.notes.map((note) => (
                            <tr key={note.id}>
                                <td>{note.title}</td>
                                <td style={textTruncateStyle} title={note.text}>{note.text}</td>
                                <td>
                                    <Button variant="outline-primary" size="sm" style={{ marginRight: "10px" }} onClick={() => {/* Handle edit logic here */}}>
                                        <FiEdit />
                                    </Button>
                                    <Button variant="outline-danger" size="sm" style={{ marginRight: "10px" }} onClick={() => this.handleDelete(note.id)}>
                                        <FiTrash2 />
                                    </Button>
                                    <Button variant="outline-success" size="sm" onClick={() => this.handleCreateFlashcard(note)}>
